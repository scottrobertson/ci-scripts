...
echo "Set Image: deployment/$DEPLOYMENT" "$DEPLOYMENT=$DOCKER_IMAGE:$BUILDKITE_COMMIT"
./kubectl set image "deployment/$DEPLOYMENT" "$DEPLOYMENT=$DOCKER_IMAGE:$BUILDKITE_COMMIT"

if [ -n "$DEPLOYMENT_2" ]; then
  echo "Set Image: deployment/$DEPLOYMENT_2" "$DEPLOYMENT_2=$DOCKER_IMAGE:$BUILDKITE_COMMIT"
