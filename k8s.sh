...
echo -e "${YELLOW}Version 18"

# Login to Docker
if [ -n "$GH_DOCKER_TOKEN" ]; then
  echo -e "${YELLOW}Using Docker Login: Github"
  echo $GH_DOCKER_TOKEN | docker login -u scottrobertson --password-stdin $DOCKER_REPO
else
  echo -e "${YELLOW}Using Docker Login: Google"
...
echo ''
echo -e "${YELLOW}Building: $DOCKER_IMAGE"
docker build --cache-from "$DOCKER_IMAGE:latest" -t "$DOCKER_IMAGE:latest" -t "$DOCKER_IMAGE:$BUILDKITE_COMMIT" .

echo ''
echo -e "${YELLOW}Pushing: $DOCKER_IMAGE:latest"
docker push "$DOCKER_IMAGE:latest"

echo ''
echo -e "${YELLOW}Pushing: $DOCKER_IMAGE:$BUILDKITE_COMMIT"
docker push "$DOCKER_IMAGE:$BUILDKITE_COMMIT"

# Install kubectl

echo ''
echo -e "${YELLOW}Installing kubectl"
curl -LO https://storage.googleapis.com/kubernetes-release/release/$(curl -s https://storage.googleapis.com/kubernetes-release/release/stable.txt)/bin/linux/amd64/kubectl && chmod +x ./kubectl

# Lets tell k8s about the new image
echo ''
echo -e "${YELLOW}kubectl set image deployment/$DEPLOYMENT $DEPLOYMENT=$DOCKER_IMAGE:$BUILDKITE_COMMIT"
...
if [ -n "$DEPLOYMENT_2" ]; then
  echo ''
  echo -e "${YELLOW}kubectl set image deployment/$DEPLOYMENT_2 $DEPLOYMENT_2=$DOCKER_IMAGE:$BUILDKITE_COMMIT"
...
echo ''
echo -e "${YELLOW}Waiting for deploy: $DEPLOYMENT"
./kubectl rollout status deployment -w -n default $DEPLOYMENT

if [ -n "$DEPLOYMENT_2" ]; then
  echo ''
  echo -e "${YELLOW}Waiting for deploy: $DEPLOYMENT_2"
